...
class Renderer {
  ...
  renderScreen() {
    if (this.game.status === "end screen") {
      ...
      let cornerSquares = this.getVisibleSquares(this.game.origin, this.game.currentLevel);
  ...
  renderMessage(messages) {
    let realArrays = [this.game.origin, this.game.robot.pos];
    let topRow = this.game.getTopRow(realArrays);
    let bottomRow = this.game.getBottomRow(realArrays);
    let leftCol = this.game.getLeftColumn(realArrays);
    let rightCol = this.game.getRightColumn(realArrays);
    for (let i = 0; i < messages.length; i++) {
      if (messages[i].left <= leftCol && messages[i].right >= rightCol
      && messages[i].top <= topRow && messages[i].bottom >= bottomRow) {
        let addY = messages[i].pos === "bottom" ? 295 : 0;
        this.drawRectangle({
          x: 5,
          y: 5 + addY,
          width: 590,
  ...
  displayLoadScreen() {
    ...
    let loadGame = window.setInterval(function () {
  ...
  displayMenu() {
    ...
    const menuRobot = new Robot([413.5, 120.5]);
    menuRobot.height = 70;
  ...
  getVisibleSquares(origin, currentLevel) {
    let topRow = Math.floor(origin[1] / BLOCK_LENGTH);
    if (topRow === -1) { topRow = 0; }
    let bottomRow = Math.floor((origin[1] + (BLOCK_LENGTH * 6) - 1) / BLOCK_LENGTH);
    if (bottomRow >= this.game.currentLevel.foregroundGrid.length) { bottomRow = this.game.currentLevel.foregroundGrid.length - 1; }
    let leftCol = Math.floor(origin[0] / BLOCK_LENGTH) - 1;
    if (leftCol < 0) { leftCol = 0; }
    let rightCol = Math.floor((origin[0] + (BLOCK_LENGTH * 8) - 1) / BLOCK_LENGTH) + 1;
  ...
  renderForeground(origin, currentLevel, cornerSquares) {
  ...
  renderBackground(origin, currentLevel, cornerSquares) {
    let rowTopY = cornerSquares[0] * BLOCK_LENGTH;
    for (let row = cornerSquares[0]; row <= cornerSquares[2]; row++) {
      let colLeftX = cornerSquares[1] * BLOCK_LENGTH;
      for (let col = cornerSquares[1]; col <= cornerSquares[3]; col++) {
  ...
  renderElevators(origin, currentLevel, cornerSquares) {
    let colLeftX = cornerSquares[1] * BLOCK_LENGTH;
  ...
  drawElevator(xBlock, adjustedPlatformTop, relBaseBottomY, exit) {
  ...
  renderBackgroundObjects(objects, origin, currentLevel, cornerSquares) {
  ...
  drawHead(robot) {
    const x = robot.pos[0] + 10;
    const y = robot.pos[1] + 10 - robot.height;
  ...
  drawDoor(door, pos) {
    ...
    if (door.status !== "open") {
      const x = pos[0];
      const y = pos[1];
      const width = Math.floor(BLOCK_LENGTH / 3);
      let topLeftCorner;
      let topRightCorner;
      ...
      let openSpace = (BLOCK_LENGTH - 1) * door.percentOpen;
      let doorHalfHeight = ((BLOCK_LENGTH - 1) - openSpace) / 2;
      if (doorHalfHeight < -1) { doorHalfHeight = -1; }
      let negPercent = (door.percentOpen - 1);
      let negToothHeight = 0;
      if (negPercent >= 0) {
      ...
      let topHeight = topLeftCorner[1] + doorHalfHeight + TOOTH_HEIGHT - negToothHeight;
  ...
  drawPlatform(pos, topColor, bottomColor) {
    const x = pos[0];
    const y = pos[1];
    const height = Math.floor(BLOCK_LENGTH / 3);
    const grad = this.c.createLinearGradient(x, y, x, y + height);
  ...
  drawWireJunction(pos, wireJunction) {
  ...
  drawCubby(pos, cubby) {
    ...
    const x2 = x + CUBBY_THICKNESS;
    const y2 = y + CUBBY_THICKNESS;
    this.drawRectangle({
      x: x2,
      y: y2,
      width: 34,
    ...
    if (cubby.item) {
      ...
      const CUBBY_DEPTH = 6;
      const x3 = x2 + CUBBY_DEPTH;
      const y3 = y2 + CUBBY_DEPTH;
      this.drawRectangle({
        x: x3,
        y: y3,
        width: 22,
  ...
  drawBrick(pos, color, leftEdges) {
    const x = pos[0];
    const y = pos[1];
    const rowHeight = (BLOCK_LENGTH / 4);
    this.drawOuterSquare(pos, color, color);
    this.c.strokeStyle = '#000';
  }

  drawButtonBlock(buttonBlock, pos) {
    ...
    const buttonPanelY = pos[1] + ((BLOCK_LENGTH - this.BUTTON_PANEL_HEIGHT) / 2) + 0.5;
    const grad = this.c.createLinearGradient(
      buttonPanelX,
      buttonPanelY,
      buttonPanelX,
  ...
  drawSpringPowerUp(pos) {
    ...
    const rightMiddleDotPos = [pos[0] + 25 + 12.5 + 8, pos[1] + 15 + 5 + 17.5];
    const leftMiddleDotPos = [pos[0] + 25 + 4.5, pos[1] + 15 + 5 + 17.5];
  ...
  drawForceFieldBlock(pos, FFBlock) {
    this.drawPowerBlock(pos, FFBlock.hasPower);
    const shellColor = '#D5D4C9';
  ...
  drawForceLine(pos) {
    const fill = this.seconds % 2 === 0 ? '#fff' : 'blue';
    this.drawRectangle({
      x: pos[0] - 2,
      y: pos[1],
      width: 4,
  ...
  drawPowerBlock(pos, power) {
    const grad = power ? this.gradientArray[this.gradientIndex] : '#333';
  ...
  drawBlock(pos) {
    const x = pos[0];
    const y = pos[1];
    const frontGrad = this.c.createLinearGradient(x, y, x, y + 75);
    frontGrad.addColorStop(0, '#2c2929');
    frontGrad.addColorStop(1, '#161515');
    const backGrad = this.c.createLinearGradient(x, y, x, y + 75);
  ...
  drawOuterSquare(pos, stroke, fill) {
    this.drawRectangle({
      x: pos[0],
      y: pos[1],
  ...
  drawRectangle(object) {
    ...
    const stroke = object.stroke || '#000';
    const fill = object.fill || undefined;
  ...
  fillGradientArray(rgbColor, arrayLength) {
